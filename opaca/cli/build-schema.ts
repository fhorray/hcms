...
export async function main() {
  ...
  lines.push(`// Instead edit your collections and run \`opaca dev\` or \`bun run opaca:build\``);
  lines.push(`import * as config from "@opaca-config";`);
  ...
  for (const col of Object.values(config.collections)) {
    lines.push(
      `export const ${col.slug} = buildDrizzleTable(config.serverConfig.collections["${col.slug}"], "${dialect}");`
    );
  }
