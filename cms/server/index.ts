...
import type { TableCmsSchemaTyped, CmsFromSchema } from "../schema/buid-schema";

// ---------------------------------------------
// Tipos de configuração
// ---------------------------------------------
type Db = ReturnType<typeof import("@/server/db").getDb>;
...
// ---------------------------------------------
// Fábrica do CRUD (GENÉRICO)
// ---------------------------------------------
export function createHonoCrudRouter<
...
>(opts: CrudOptions<TSchema, TCms>) {
  ...
  const tableNames = Object.values(opts.schema)
    ...
    .filter((name) => !!opts.cms[name as keyof TCms]);
  ...
  app.get(`${base}/:table`, async (c) => {
    ...
    const cmsTable = opts.cms[tableName as keyof TCms] as unknown as TableCmsSchemaTyped<any, any>;
    const cfg = opts.tables?.[tableName];

    if (cfg?.acl?.canList && !(await cfg.acl.canList({ c }))) return c.json({ error: "Forbidden" }, 403);

    // paginação, ordenação, projeção
    const q = c.req.query();
